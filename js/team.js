/**
 * TEAM RENDERER
 * Menangani tampilan tim energi — data dari Firestore via window.__teamData
 */

function getTeamData() {
    if (window.__teamData && window.__teamData.length > 0) {
        return [...window.__teamData].sort((a, b) => (a.order || 999) - (b.order || 999));
    }
    return [];
}
...
export function renderTeam() {
    ...
    if (teamData.length === 0) {
        container.innerHTML = '<div class="text-center text-slate-500 py-20 text-sm italic">Data tim belum tersedia. Silakan isi via Admin Panel.</div>';
        return;
    }

    const createCard = (person, delay) => {
        const imagePath = person.image || 'images/default.jpg';
        const link = person.portfolioLink || person.link || '';
        const isClickable = !!link;

        const wrapperTag = isClickable ? 'a' : 'div';
        const linkAttr = isClickable ? `href="${link}" target="_blank" rel="noopener noreferrer"` : '';
        ...
        <${wrapperTag} ${linkAttr} class="block luxury-card group w-[220px] md:w-[240px] p-6 text-center z-10 relative transition-transform duration-500 ${cursorClass}" style="animation: fadeInUpTeam 0.6s ease backwards ${delay}s;">
            ...
            <div class="hex-container relative z-10 mx-auto">
                ...
                <div class="hex-photo-wrap">
                    <img src="${imagePath}" alt="${person.name}" loading="lazy"
    ...
    let html = '<div class="flex flex-col items-center w-full relative py-6 max-w-5xl mx-auto overflow-x-hidden">';

    // First = top (Manager)
    if (teamData[0]) html += createCard(teamData[0], '0.1');

    // Connector + second = below (Supervisor)
    if (teamData[1]) {
        html += `
        ...
        html += `
            <div class="hidden md:block w-full max-w-[800px] relative">
                ...
                <div class="grid grid-cols-3 w-full">
                    ${rest.map(() => '<div class="flex justify-center"><div class="w-[2px] h-10 relative overflow-hidden line-glow"><div class="absolute inset-0 energy-flow-y"></div></div></div>').join('')}
                </div>
            </div>
        `;

        html += '<div class="flex flex-col items-center md:grid md:grid-cols-3 w-full max-w-[800px] gap-0 md:gap-4 justify-items-center relative z-10 pb-10">';
        const delays = ['0.5', '0.7', '0.9'];
        rest.forEach((person, i) => {
            if (i > 0) html += mobileLine;
            html += createCard(person, delays[i % delays.length]);
        });
        html += '</div>';
    }
